...
- (void) enable:(CDVInvokedUrlCommand*)command {
  ...
  if (self.sessionManager != nil) {
    ...
    //    self.storeToFile = (BOOL) [command.arguments[11] boolValue];
        ...
        self.cameraRenderController.view.frame = CGRectMake(0, 0, screenWidth, screenHeight);
        self.cameraRenderController.delegate = self;

        [self.viewController addChildViewController:self.cameraRenderController];
          
          NSDictionary *setupSessionOptions;
          NSString* cameraDirection = @"back";
          NSNumber* targetSize = 0;
          
          if (command.arguments.count > 0) {
              for(int i = 0; i < command.arguments.count; i ++) {
                  NSDictionary* config = command.arguments[i];
                  
                  for (NSString *key in config) {
                      if ([key isEqual:@"targetSize"]) {
                          targetSize = [NSNumber numberWithInteger:((NSNumber*)config[key]).intValue];
                      } else if ([key isEqual:@"direction"]) {
                          cameraDirection = config[key];
                      }
                  }
              }
          }
          ...
          if (targetSize > 0) {
              setupSessionOptions = @{ @"targetSize" : targetSize };
          } else {
    ...
    [self.sessionManager setupSession:cameraDirection completion:^(BOOL started) {
...
- (void) switchCamera:(CDVInvokedUrlCommand*)command {
  ...
  CDVPluginResult *pluginResult;
    ...
    if (command.arguments.count > 0) {
        NSString* config = command.arguments[0];
        
        if (self.sessionManager != nil) {
            [self.sessionManager switchCamera:config completion:^(BOOL switched) {
                [self.commandDelegate sendPluginResult:[CDVPluginResult resultWithStatus:CDVCommandStatus_OK] callbackId:command.callbackId];
            }];
...
- (void) setFlashMode:(CDVInvokedUrlCommand*)command {
  ...
  if (self.sessionManager != nil) {
    if ([flashMode isEqual: @"off"]) {
        [self.sessionManager setFlashMode:AVCaptureFlashModeOff photoSettings:self.photoSettings];
    } else if ([flashMode isEqual: @"on"]) {
      [self.sessionManager setFlashMode:AVCaptureFlashModeOn photoSettings:self.photoSettings];
    } else if ([flashMode isEqual: @"auto"]) {
      [self.sessionManager setFlashMode:AVCaptureFlashModeAuto photoSettings:self.photoSettings];
...
- (void) capture:(CDVInvokedUrlCommand*)command {
  ...
  if (self.cameraRenderController != NULL) {
    ...
    [self.sessionManager setFlashMode:[useFlash isEqual: @"ON"] ? 1 : [useFlash isEqual: @"OFF"] ? 0 : 2  photoSettings:self.photoSettings];
...
//        CGImageRef capturedImageRef = CGImageSourceCreateImageAtIndex(source, 0, NULL);
//       
//        
//        if (self.sessionManager.defaultCamera == AVCaptureDevicePositionFront) {
//            CIImage *capturedCImage = [[CIImage alloc] initWithCGImage:capturedImageRef];
//            
//            
//            
//            CGAffineTransform matrix = CGAffineTransformTranslate(CGAffineTransformMakeScale(1, -1), 0, capturedCImage.extent.size.height);
//            CIImage *imageToFilter = [capturedCImage imageByApplyingTransform:matrix];
//            
//            int i = 1;
//            
//            i += 1;
...
//        CGImageRelease(capturedImageRef);
//        
...
//        [self.commandDelegate sendPluginResult:pluginResult callbackId:self.onPictureTakenHandlerId];
        ...
        capturedCImage = [[CIImage alloc] initWithCGImage:capturedImageRef];
       ...
       CGImageRef finalImage = [self.cameraRenderController.ciContext createCGImage:imageToFilter fromRect:imageToFilter.extent];
       ...
       CDVPluginResult *pluginResult;
        ...
        NSData *data = UIImageJPEGRepresentation([UIImage imageWithCGImage:resultFinalImage], 1.0);
...
//- (void) invokeTakePicture:(CGFloat) quality{
...
//      } else {
//
//        NSData *imageData = [AVCapturePhotoOutput jpegStillImageNSDataRepresentation:sampleBuffer];
