...
- (void) setupSession:(NSString *)defaultCamera completion:(void(^)(BOOL started))completion{
  ...
  dispatch_async(self.sessionQueue, ^{
      ...
      if ([self.session canAddOutput:stillImageOutput]) {
        ...
        self.imageOutput = stillImageOutput;
      }
...
- (void) switchCamera:(NSString*) direction completion:(void(^)(BOOL switched))completion {
    if ([direction isEqualToString:@"front"] && self.defaultCamera == AVCaptureDevicePositionFront) {
        completion(TRUE);
        
        return;
    }
    
    if ([direction isEqualToString:@"back"] && self.defaultCamera == AVCaptureDevicePositionBack) {
        completion(TRUE);
